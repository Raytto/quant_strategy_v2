...
#### 2.1.1 港股基础表 hk_basic (对应本地 `stock_basic_h`)
来源: TuShare 文档 doc_id=191 (接口名 hk_basic)，可一次性拉取全部在交易或指定状态的港股代码。下表为核心字段精要说明（基于官方字段释义的整理/概括，仅保留与量化/映射相关的关键信息）：

| 字段 | 含义/用途概述 | 典型使用 | 备注 | 
|------|---------------|----------|------|
| ts_code | TS 统一代码 (形如 00001.HK) | 作为主键/联结 | 与日线、复权表对齐 |
| name | 股票简称（多为中文或常用简称） | 展示 / A-H 名称模糊匹配 | 可能与 A 股简称存在轻微差异 |
| fullname | 公司全称 | 精准匹配 / 报表拼接 | 英文/繁体差异需注意 |
| enname | 英文名称 | 跨语言展示 / 海外对照 | 有助于跨市场统一 |
| cn_spell | 简称拼音 | 构造搜索 / 模糊匹配 | A 股为 `cnspell` 命名差异 |
| market | 市场类别 (主板/创业板等) | 分层统计 / 过滤 | 细分板块回测分组 |
| list_status | 上市状态: L=上市, D=退市, P=暂停 | 过滤有效代码 | 回测/研究多用 L |
| list_date | 上市日期 | IPO 剔除、样本生存偏差处理 | 用于设定可交易起点 |
| delist_date | 退市日期 | 生存偏差校正 | 为空表示仍在交易 |
| trade_unit | 交易单位 (手/股) | 成交量换算/最小下单单位校验 | 部分个股为每手 N 股 |
| isin | ISIN 国际代码 | 跨数据库映射 (如数据供应商) | 可与外部财务/ESG 数据桥接 |
| curr_type | 货币代码 (多为 HKD) | 多币种资产估值 | 与汇率表结合 (HKD→CNY) |

补充说明:
- 港股拼音字段命名差异需在自动映射脚本中特判，避免直接与 A 股 `cnspell` 混用。
- 若需建立更稳健的 A/H 对应关系，可综合比较 `name` 前缀、`fullname` 关键字、上市日期等，或维护手工映射表（当前项目使用 `data/ah_codes.csv`）。
- 对历史退市或暂停的代码，如需回测生存偏差校正，应联合 `list_date`/`delist_date` 过滤出当期真实可交易集合。
- `trade_unit` 在真实撮合或最小头寸估算中可用于避免出现非整手（如果做精细化模拟）。

#### 2.1.2 A股基础表 stock_basic (对应本地 `stock_basic_a`)
来源: TuShare 文档 doc_id=25 (接口名 stock_basic)。一次性拉取全量基础属性后本地缓存，增量仅需补新增上市代码。以下为量化/映射常用字段精要 (已再表述，非逐字复制官方释义)：

| 字段 | 含义/用途概述 | 典型使用 | 备注 |
|------|---------------|----------|------|
| ts_code | TS 统一代码 (如 000001.SZ) | 主键/联结行情、复权、财务 | 与其他表 (daily_a, adj_factor_a) 对齐 |
| symbol | 交易所代码纯数字部分 | 与外部数据/券商终端匹配 | 深/沪前缀靠 ts_code 后缀判断 |
| name | 股票简称 | 展示 / 模糊匹配 / A-H 对照 | 可能发生简称变更 (需结合曾用名接口) |
| area | 地域 | 区域分组、监管事件筛选 | 受行政区划调整影响较小 |
| industry | 行业分类 | 因子/横截面分组 | 分类口径可能随时间更新 (需留时间戳快照) |
| fullname | 公司全称 | 精确匹配、公告抓取 | 与英文/拼音区分 |
| enname | 英文全称 | 跨语言、海外数据桥接 | 部分可能缺失或格式不一 |
| cnspell | 简称拼音缩写 | 名称搜索/映射 | 港股对应字段为 cn_spell (命名差异) |
| market | 市场类型 (主板/创业板/科创板/CDR/北交所) | 分层统计/策略分段 | 可用于限制策略适用范围 |
| exchange | 交易所代码 (SSE/SZSE/BSE) | 多市场过滤 | 与 ts_code 后缀重复但更结构化 |
| curr_type | 交易货币 (一般 CNY) | 多币种估值 (极少用) | A 股统一 CNY 基本无差异 |
| list_status | 上市状态 L/D/P | 剔除退市或暂停 | 回测生存偏差处理关键字段 |
| list_date | 上市日期 | IPO 过滤 / 样本起始对齐 | 设定最少上市天数约束 |
| delist_date | 退市日期 | 生存偏差校正 | 为空/None 表示仍在交易 |
| is_hs | 沪深港通标记 (N/H/S) | 港通成分过滤 | 与互联互通资金流策略相关 |
| act_name | 实控人名称 | 治理/事件研究 | 变化需留意公告解析 |
| act_ent_type | 实控人企业性质 | 机构属性因子/风险分类 | 分类标准可能调整 |

补充说明:
- 建议本地保持“快照 + 变更日志”机制以追踪行业/简称等随时间变化的字段；否则历史回测时存在回看偏差。
- `is_hs` 可与港股表结合，过滤 A/H 同时互联互通标的形成更窄的可交易对。
- 批量拉取推荐字段子集: `ts_code,symbol,name,area,industry,list_date,list_status,is_hs`，其余在需要时再补。
